 ...
 * http://book2s.com/java/src/package/com/avast/syringe/config/fm/propertiesloader.html#82f23f829d3f167ece65bde054ae4bd8
 * User: slajchrt
 * Date: 4/11/12
 * Time: 12:46 PM
 */
public class PropertiesLoader {
    ...
    private static void parseProperty(Map.Entry property, Map map) {
        ...
        Preconditions.checkArgument(!key.contains("."),
                "Avoid using dots in property %s", key);

        Object value;
        if (key.endsWith("]")) {
            int i = key.indexOf('[');
            Preconditions.checkArgument(i > 0,
                    "Invalid index specification in property %s", key);
            ...
            Preconditions.checkArgument(propVal[index] == null,
                    "Duplicate index %s in property %s", index, key);
